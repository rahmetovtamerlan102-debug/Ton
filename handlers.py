    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            await status_msg.edit_text(
                report, parse_mode=ParseMode.HTML, reply_markup=keyboards.main_keyboard(),
        ...
        try:
            if section == "main":
                ...
                await query.edit_message_text(
                    text, parse_mode=ParseMode.HTML, reply_markup=keyboards.main_keyboard(),
                    disable_web_page_preview=True,
                )
                return
            ...
            await query.edit_message_text("❌ Аккаунт не найден.", reply_markup=keyboards.main_keyboard())
        except tonapi.TonApiError as e:
            await query.edit_message_text(f"❌ TonAPI временно недоступен ({e}).", reply_markup=keyboards.main_keyboard())
        except Exception as e:
            logger.exception("Unexpected error in callback")
            await query.edit_message_text(f"❌ Ошибка: {e}", reply_markup=keyboards.main_keyboard())
